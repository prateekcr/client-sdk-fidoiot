...
bool fdo_rvto2addr_read(sdor_t *sdor, fdo_rvto2addr_t *rvto2addr) {
	...
	LOG(LOG_DEBUG, "RVTO2Addr: There are %zu RVTO2AddrEntry\n", num_rvto2addr_items);
	...
	if (!entry) {
		LOG(LOG_ERROR, "RVTO2Addr: Failed to alloc RVTO2AddrEntry(s)\n");
		return false;	
	}
	size_t i = 0;
	for (;;) {
	...
	// fdo_rvto2addr_entry_free(entry);
	return false;
}
